...
int send_string_udp(char *buf_send, int size,int sock_desc,struct addrinfo *serv_info)
{
	int send_size=0;
			...
			send_size=sendto(sock_desc, buf_send, size, 0,serv_info->ai_addr, serv_info->ai_addrlen); 
			if(send_size<=0) {
				perror("sendto UDP:");
				return -1;	
					}
...
int receive_string_udp(char *buf_recv,int size,int sock_desc, struct addrinfo *serv_info)
{
	...
	sin_size=sizeof (their_addr);
			...
			receive_size=recvfrom(sock_desc,buf_recv, size, 0,(struct sockaddr *)&their_addr, &sin_size); 
			if(receive_size<=0) {
				perror("recvfrom UDP:");
				return -1;	
					}
