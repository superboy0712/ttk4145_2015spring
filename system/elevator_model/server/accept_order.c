...
void* accept_order_function(void *shared_interface_data){
	...
	my_interface_t* interface_in_order_thrd=(my_interface_t* ) shared_interface_data;
		...
		//////////////Receiving new data/////////////////
				for(loop_var=0;loop_var<N_CLIENT;loop_var++){
					...
					if(FD_ISSET(local_nodes_data[loop_var].sock_order, &order_set)){
						...
						recv_bytes=recv(local_nodes_data[loop_var].sock_order, local_nodes_data[loop_var].buf_read, SEND_SIZE, 0);
						...
						if(recv_bytes<=0){
							...
							printf("Accept Order Thread: Something wrong with this socket,client  %s should be closed %d bytes received.\n",local_nodes_data[loop_var].my_ip,recv_bytes);
								...
								/*Debug part only*/
								if(!strncmp(local_nodes_data[loop_var].buf_read, "DOJOB_", 6)){;}
								else {printf("\t\t\tAccept Order Thread:WRONG Message for status from %s\n",local_nodes_data[loop_var].my_ip);}
								///////////////////
			
							subString(local_nodes_data[loop_var].buf_read, 6, 1, temp_floor_in_str);
							printf("%s", temp_floor_in_str);
							temp_floor_in_str[1] = '\0';
							temp_received_floor=atoi(temp_floor_in_str);

							subString(local_nodes_data[loop_var].buf_read, 8, 1, temp_received_direction);
							temp_received_direction[1] = '\0';

							/*Debug Part*/
						printf("\t\t\tAccept Order Thread: Order Received; message is %s Floor is %d from %s\n.",local_nodes_data[loop_var].buf_read,temp_received_floor,local_nodes_data[loop_var].my_ip);

							pthread_mutex_lock(&interface_in_order_thrd->interface_mutex);

							interface_in_order_thrd->received_floor=temp_received_floor;
							interface_in_order_thrd->received_direction=temp_received_direction[0];
							interface_in_order_thrd->received_floor_flag=TRUE;

							pthread_mutex_unlock(&interface_in_order_thrd->interface_mutex);
