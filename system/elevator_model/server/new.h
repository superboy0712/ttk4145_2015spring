...
#define DELAY			200
	...
	int update_status_function(int *sock_desc,char *my_ip, char *data_send, char *data_recv);
	char* subString (const char* input, int offset, int len, char* dest);
	void makeStatus (char *output, int floor, char dir);
	int cost_function(struct cost_param, int temp_order_floor, char temp_order_direction);
