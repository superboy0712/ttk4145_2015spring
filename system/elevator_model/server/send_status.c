...
void* send_status_function(void *shared_interface_data) {
	...
	my_interface_t* interface_in_status_thrd =
	...
	while (1) {
		...
		if (i == -1) {
			printf("Send Status TCP Thread:Select Failed\n");
		}
		...
		for (loop_var = 0; loop_var < N_CLIENT; loop_var++) {
			...
			if (FD_ISSET(local_nodes_data[loop_var].sock_status, &recv_set)) {
				...
				recv_bytes = recv(local_nodes_data[loop_var].sock_status,
				...
				if (recv_bytes <= 0) {
					if (recv_bytes == 0) {
						...
						printf(
								"Send Status TCP Thread:Client %s disconnected on recv error closing all sockets %d bytes recvived.\n",
						...
						printf(
								"Send Status TCP Thread:Deleting %s ip SUCCESS.\n",
								local_nodes_data[loop_var].my_ip);			//}
					...
					if (!strncmp(local_nodes_data[loop_var].buf_read,
						...
						printf(
								"\t\t\tSend Status TCP Thread:WRONG Message for status\n");
					}
					///////////////////
					pthread_mutex_lock(
							&interface_in_status_thrd->interface_mutex);

					strncpy(temp_buf_interface,
							interface_in_status_thrd->interface_status_buffer,
							SEND_SIZE);

					pthread_mutex_unlock(
							&interface_in_status_thrd->interface_mutex);

					send_bytes = send(local_nodes_data[loop_var].sock_data,
							temp_buf_interface, SEND_SIZE, 0);

					if (send_bytes <= 0) {

						printf(
								"Send Status TCP Thread:Sending status failed on data channel for %s \n",
								local_nodes_data[loop_var].my_ip);
						...
						printf(
								"Send Status TCP Thread: Status %s successfully sent to client %s.\n",
								temp_buf_interface,
								local_nodes_data[loop_var].my_ip);
