...
while(1)
{
	...
	if(rand_floor==1 || rand_floor==3) {rand_dir='U';}
...
/*	pthread_mutex_lock(&in_main_interface->interface_mutex);
	strncpy(in_main_interface->interface_status_buffer,"MY_STATUS_0_D",13);
	strncpy(buf,in_main_interface->interface_status_buffer,13);
...
*/
	sprintf(buf, "MY_STATUS_%d_%c", rand_floor,rand_dir);
...
/*			printf("Enter Floor\n");
			...
			strncpy(in_main_interface->interface_status_buffer,buf,13);
			...
			pthread_mutex_lock(&in_main_interface->interface_mutex);
				...
				while(1){
						...
						if(	in_main_interface->order_floor_flag==FALSE){printf("Main:Order Flag Cleared by comm module....\n");temp_order_floor=in_main_interface->order_floor;pthread_mutex_unlock(&in_main_interface->interface_mutex);break;}

						else	{pthread_mutex_unlock(&in_main_interface->interface_mutex); usleep(DELAY*MS);}
				}
