									...
									pthread_mutex_unlock(&new_connection_data.ip_mutex);		//edited part
										...
										if(ip_check_var==-1){		//-1 means not in list
											...
											temp_desc_order=connect_function(recv_ip, order_port);
												...
												if(temp_desc_status!=-1 && temp_desc_data!=-1 && global_ip_index<=N_CLIENT){

													pthread_mutex_lock(&new_connection_data.ip_mutex);

													strncpy(new_connection_data.new_ip,recv_ip,INET6_ADDRSTRLEN);		//copy ip
													...
													strncpy(global_ipArray[global_ip_index],recv_ip,INET6_ADDRSTRLEN);			//add to ip array							
													...
													usleep(50*MS);
														...
														while(1){
															...
															if(new_connection_data.ip_flag==FALSE) {printf("\t\t\tUDP Thread:New Connection Added Successfully\n"); pthread_mutex_unlock(&new_connection_data.ip_mutex); break;}

															else pthread_mutex_unlock(&new_connection_data.ip_mutex);
										...
										else pthread_mutex_unlock(&new_connection_data.ip_mutex);		//release ip_mutex
...
void* tcp_accept_function()
{
	...
	for(loop_var=0; loop_var<N_CLIENT; loop_var++){
		...
		memset(connected_nodes_data.clients[loop_var].my_ip, 0, INET6_ADDRSTRLEN);
...
/////////////////////////////////////////////////////////////
  ...
  char remoteIP[INET6_ADDRSTRLEN];
	...
	new_connection_data.fdmax= (sock_data > sock_status) ? sock_data : sock_status;			//first assign larger of sock_data or sock_status to fdmax
	new_connection_data.fdmax= (new_connection_data.fdmax > sock_order) ? new_connection_data.fdmax : sock_order;	//now assign larger of sock_order or fdmax to fdmax!!!
	...
	while(1){
		...
		select(new_connection_data.fdmax+1,&new_connection_data.sock_set,NULL,NULL,&tv);    //pass 0 instead of &tv as first correction to timeout issue
	...
	//////////////Accepting new connection on sock_status  using select/////////////////
		    ...
		    if(FD_ISSET(sock_status,&new_connection_data.sock_set)){
						...
						temp_desc_status=accept(sock_status,(struct sockaddr *)&remoteaddr,&addrlen);
					...
					//now add new connection to set of socks to poll
						...
						pthread_mutex_unlock(&new_connection_data.ip_mutex);
								...
								if(temp_index!=-1){		//this means already in list, so no need to incerment client count, just add sock_desc!!!!
										...
										connected_nodes_data.clients[temp_index].sock_status=temp_desc_status;	//only sock_desc, NOT ip since already exisitng!
										...
										connected_nodes_data.clients[client_connected].sock_status=temp_desc_status;
										strncpy(connected_nodes_data.clients[client_connected].my_ip, remoteIP, INET6_ADDRSTRLEN);

										pthread_mutex_unlock(&connected_nodes_data.node_mutex);		 //unlock node mutex

										pthread_mutex_lock(&new_connection_data.ip_mutex);		//for set operations, ask YB whether this is correct......

										strncpy(global_ipArray[global_ip_index], remoteIP, INET6_ADDRSTRLEN);
			...
			else pthread_mutex_unlock(&new_connection_data.ip_mutex);		//unlock_mutex for both of if(ISSET.....
	...
	//////////////Accepting new connection on sock_data  using select/////////////////
		   ...
		   if(FD_ISSET(sock_data,&new_connection_data.sock_set)){
						...
						temp_desc_data=accept(sock_data,(struct sockaddr *)&remoteaddr,&addrlen);
					...
					//now add new connection to set of socks to poll
						...
						pthread_mutex_unlock(&new_connection_data.ip_mutex);
								...
								if(temp_index!=-1){		//this means already in list, so no need to incerment client count, jus add sock_desc!!!!
										...
										connected_nodes_data.clients[temp_index].sock_data=temp_desc_data;		//only sock_desc, NOT ip since already exisitng!*/
										...
										connected_nodes_data.clients[client_connected].sock_data=temp_desc_data;
										strncpy(connected_nodes_data.clients[client_connected].my_ip, remoteIP, INET6_ADDRSTRLEN);
	...
	//////////////Accepting new connection on sock_order  using select/////////////////
		   ...
		   if(FD_ISSET(sock_order,&new_connection_data.sock_set)){
						...
						temp_desc_order=accept(sock_order,(struct sockaddr *)&remoteaddr,&addrlen);
					...
					//now add new connection to set of socks to poll
						...
						pthread_mutex_unlock(&new_connection_data.ip_mutex);
								...
								if(temp_index!=-1){		//this means already in list, so no need to incerment client count, jus add sock_desc!!!!
										...
										connected_nodes_data.clients[temp_index].sock_order=temp_desc_order;		//only sock_desc, NOT ip since already exisitng!
										...
										connected_nodes_data.clients[client_connected].sock_order=temp_desc_order;
										strncpy(connected_nodes_data.clients[client_connected].my_ip, remoteIP, INET6_ADDRSTRLEN);
	...
	//////////////Adding new connection from udp thread/////////////////
			...
			pthread_mutex_lock(&new_connection_data.ip_mutex);		//lock new_connection_data mutex
				...
				else pthread_mutex_unlock(&new_connection_data.ip_mutex);		//unlock new_connection_data mutex if not true...
