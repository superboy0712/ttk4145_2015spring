...
pthread_t resolve_order_thread;
	...
	my_interface_t* interface_in_init_thrd=(my_interface_t* ) shared_interface_data;
...
pthread_create(&send_status_thread, NULL, send_status_function, interface_in_init_thrd);
pthread_create(&accept_order_thread, NULL, accept_order_function, interface_in_init_thrd);
pthread_create(&resolve_order_thread, NULL, resolve_order_function, interface_in_init_thrd);
...
pthread_join(resolve_order_thread, NULL);
	...
	int ip_check_function(char *ip){			//if new connectio, it will return -1 otherwise it will return 0 to N_CLIENT i.e. ip_index of that client 
		int i;
			...
			for(i=0;i<N_CLIENT;i++){
				...
				if(!strncmp(ip,global_ipArray[i],INET6_ADDRSTRLEN)) return i;		//return index; to be used in accept function!!!! 
	...
	int ip_delete_function(char *ip){
		...
		int i;
			...
			for(i=0;i<N_CLIENT;i++){
				if(!strncmp(ip,global_ipArray[i],INET6_ADDRSTRLEN)) {memset(global_ipArray[i],0,INET6_ADDRSTRLEN); return 1;}	//if ip matches, delete ip from array
	...
	int cost_function(struct cost_param_t cost_values, int temp_order_floor, char temp_order_dircetion){
